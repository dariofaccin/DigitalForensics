...
% malloc the memory
wave_trans = zeros(nr,nc); 
image_noise = zeros(M,N,C);

% for each RGB channel
for c = 1:C

    % Wavelet decomposition, without redudance
    wave_trans = mdwt(Im(:,:,c),qmf,L);
    
    % Extract the noise from the wavelet coefficients
    nr = ceil((M+minpad)/m)*m;  nc = ceil((N+minpad)/m)*m;  % dimensions of the padded image (always pad 8 pixels or more)
    for i=1:L
        % indices of the block of coefficients
    ...
    image_noise_temp=midwt(wave_trans,qmf,L);
    
    % Crop to the original size
    image_noise(:,:,c) = image_noise_temp(pr+1:pr+M,pc+1:pc+N);
end
